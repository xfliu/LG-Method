function R = run_regular_tet_ref2_lg_rigorous(p)
...
% A0,A1,A2 are assembled in interval arithmetic, not intval(double).
    ...
    log_file = sprintf('logs/regular_tet_ref2_p%d_lg_rigorous_%s.log', p, datestr(now,'yyyymmdd'));
    ...
    mesh = refine_tet_mesh(refine_tet_mesh(mesh0));   % 2 refinements -> 64 tetrahedra

    groups = {struct('idx',1,'rho',rho2), struct('idx',2:4,'rho',rho5)};
    R = run_conforming_lg_rigorous(mesh, p, n_eig, lhat, groups, ...
        sprintf('reg-64elt-P%d', p));
    R.domain = 'unit regular tetrahedron, two uniform refinements (64 tets)';
    R.rho2 = rho2; R.rho5 = rho5; R.rel_gap = (R.ub - R.lb)./R.ub*100;

    fprintf('\nCertified 64-element P%d rigorous LG bounds:\n', p);
    fprintf('  k     lower bound        upper bound        rel gap\n');
    for k = 1:n_eig
        fprintf('  %d   %.12f   %.12f   %.6f%%\n', k, R.lb(k), R.ub(k), R.rel_gap(k));
    end
    save(sprintf('regular_tet_ref2_p%d_lg_rigorous_results.mat', p), 'R', '-v7.3');
end
